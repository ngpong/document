...
- [类](#类)
  ...
  - [类的前置声明](#类的前置声明)
  - [继承](#继承)
    - [继承链中类的构造和析构顺序](#继承链中类的构造和析构顺序)
    - [继承的内存模型](#继承的内存模型)
    - [显示指定父类构造函数进行父类成员的初始化工作](#显示指定父类构造函数进行父类成员的初始化工作)
    - [继承链中的调用二义性](#继承链中的调用二义性)
    - [里式转换](#里式转换)
    - [多继承与菱形继承的问题](#多继承与菱形继承的问题)
    - [虚继承](#虚继承)
      - [如何编写虚继承](#如何编写虚继承)
      - [虚继承内部实现](#虚继承内部实现)
    - [虚函数](#虚函数)
...
- 在标准中规定，引用一个已经完成析构的实例的任何调用都是未定义的行为。在 GCC 的实现中，实例在完成析构后其成员数据并不会立即被重置，而是等到未来不久的某个时间点。在生产环境中依赖任何未定义的行为都不是维持程序健壮性的正确做法，但即便如此，我们还是可以在测试环境中实现一些有趣的尝试：
...
如果一个静态变量是直接声明在类中并赋予定义的情况，则必须添加 `const` 关键词进行修饰，否则会编译报错，简而言之就是<font color = "red">静态变量如果是在类中进行定义的话，它必须是一个静态常量</font>
...
依据前面所述，静态成员并不依赖于实例的内存，因此要使用它直接使用 `CLASS::NAME` 的语义即可；这种方式，会尝试在指定 `CLASS` 中寻找一个名为 `NAME` 的静态成员，如果未找到，则根据继承链继续往上寻找直接基类，直至查找失败而结束。

当然，引用静态成员的方式也可以像普通成员时一样通过类型的实例来引用，只是这么做并无任何优势，反而加重了解读代码时的心智负担，因为我们一般都会默认认为通过实例内存所引用的成员只是普通类成员，它们会依随实例内存的创建而初始化。
...
这里重点需要说明的是这个继承类别，一个基类可以被继承自 `public`, `protected`, `private` 这三种类别，不同的继承类别能够决定基类成员延伸至派生后的访问保护性。我们可以简单的理解为继承类别的使用可以间接改变基类成员的访问修饰符类型，<font color = "red">一般而言，我们使用继承都是用的 public 的形式来完成，因为这个形式是最直接的</font>
...
<span id = "继承链中类的构造和析构顺序"></span>

#### 继承链中类的构造和析构顺序

在[标准](https://en.cppreference.com/w/cpp/language/constructor)中规定，类在实例化的过程中必须是依据继承链（如果存在）的顺序去执行的。这段顺序要求优先调用根基类的构造函数（无任何指定的情况下则调用默认无参的构造）以将根基类中的所有成员初始化到一段内存中，然后再从这段范围（根基类所占用的内存范围）的结束地址开始继续初始化下级派生中的所有成员到内存，该执行过程会一直循环并持续到继承链的末端。这样的逐继承链从后往前构造的顺序要求符合 FIFO 的特性，也就是说，对于释放（析构）顺序是一个完全相反的调用形式，即：先完成最下层派生的析构工作，然后再向上逐层级进行析构，直至抵达继承链的顶部。
...
public:
  Chausie() /* 如没有特别指定则插入类似 Cat() 的代码以调用默认构造 */ {
    val = 30;
  }
  ~Chausie() {
    val = -30;
...
0000000000001189 <main>:
    ...
    ;; 此处调用 Chausie 的构造函数
    11b3:	e8 08 01 00 00       	call   12c0 <_ZN7ChausieC1Ev>

    ;; ......

    ;; 此处调用 Chausie 的析构函数
    1184:	e8 bb 00 00 00       	call   1244 <_ZN7ChausieD1Ev>

;; ----------------------------------------------------------

;; 此处为 Animal 的构造函数
000000000000124c <_ZN6AnimalC1Ev>:
    ;; ......
...
;; 此处为 Animal 的析构函数
00000000000011c0 <_ZN6AnimalD1Ev>:
    ;; ......
...
;; 此处为 Cat 的构造函数
0000000000001276 <_ZN3CatC1Ev>:
    ;; ......

    ;; 插入代码以调用 Animal 的构造函数
    128d:	e8 ba ff ff ff       	call   124c <_ZN6AnimalC1Ev> 
...
;; 此处为 Cat 的析构函数
00000000000011fa <_ZN3CatD1Ev>:
    ;; ......

    ;; 执行 Cat 成员的析构工作

    ;; 插入代码以调用 Animal 的析构函数
    1211:	e8 aa ff ff ff       	call   11c0 <_ZN6AnimalD1Ev>

;; ----------------------------------------------------------

;; 此处为 Chausie 的构造函数
00000000000012c0 <_ZN7ChausieC1Ev>:
    ;; ......

    ;; 插入代码以调用 Cat 的构造函数
    12d7:	e8 9a ff ff ff       	call   1276 <_ZN3CatC1Ev>
...
;; 此处为 Chausie 的析构函数
000000000000121a <_ZN7ChausieC1Ev>:
    ;; ......

    ;; 执行 Chausie 成员的析构工作

    ;; 插入代码以调用 Cat 的析构函数
    125b:	e8 9a ff ff ff       	call   11fa <_ZN3CatD1Ev>
...
```text
$ cl /d1 reportSingleClassLayout<type> <filename>
...
由于构造总是先从根基类开始的特性，我们能够发现基类中的成员数据永远都会被包含于派生类内存首部。另外一点，派生类实质上能够以无视访问修饰符的方式去继承基类中的所有成员（非构造型函数，如：构造函数, 析构函数, 等号运算符重载函数），并将它们都初始化在一块连续的内存段范围内。关键字永远只是语言抽象上的一种概念，从访问效率与对象完整性上来看，没有任何理由不允许编译器实现这样的内存布局，即便在完全同一块连续的内存段中，我们也能够在一个已经在编译期间就确定的偏移量来区分开来继承链中各类型所属成员的访问工作。

当拥有了上面的讨论基础后，我们可以进行一个有趣的实验：我们都知道，派生类所继承自基类中的成员会受到基类中成员声明的访问修饰符的影响的，也就意味着基类中某个声明为私有的成员在其派生类中是无法通过直接的方式去访问的，当然这里重点强调的是 "直接"。实际上基类中的所有成员是存储在派生类的实例其所对应的内存段当中，那么我们可以大胆的认为基类中声明为 `private` 的成员实际上并不是 `private` 的，因为我们完全可以通过一个派生类的内存首地址，通过地址偏移的方式找到那个声明为 `private` 的在基类的成员其在内存中的首地址，然后通过解引用的方式以操作它。
...
派生类在实例化的过程中会优先初始化从基类中继承下来的成员，成员的初始化过程必定要涉及到构造函数的调用。如在没有显式指定任何基类构造的情况下，编译器都会调用基类中的默认构造函数。这个特殊的构造函数没有参数，且在我们没有为类定义任何构造函数的情况下才会由编译器自动创建。那么当基类的默认构造在创建失败的情况下，派生类要还是保持原始构造语义的调用则会产生编译错误。为解决这一办法，只需在基类中手动添加一个默认且无参的构造函数即可解决。但是更推荐的做法则是在派生类构造函数的 [初始化列表](#构造函数中的初始化参数列表) 中显式指定（调用）基类中已有的构造函数。

需要注意的是，这要求我们尽量保证显式指定基类的构造函数调用要放在初始化列表的最前面。虽然不遵守这一规则也能够编译通过且不会产生任何影响，但是为了代码健壮性我们还需要做出更多的努力工作，即代码语义上也同时保留继承层次上的初始化顺序。
...
<span id = '继承链中的调用二义性'></span>

#### 继承链中的调用二义性

在一个拥有继承层次的类中，我们要保证成员之间的调度不会出现二义性的问题，对于：

- 派生类和基类中存在同名成员时，派生类对于同名成员的调用会存在二义性。编译器会直接隐藏掉基类中会因为此次调用而造成二义性冲突的成员（如果成员类型为函数，则隐藏掉其包括所有重载的版本）。

- 派生类所继承的多个基类之间存在同名成员时，派生类对于同名成员的调用会存在二义性。由于编译器并不知道此次调用的所属来源（多个基类之间都存在同名成员），故针对这种情况下编译器会直接报错。

- 当派生类中同时存在以上两种二义性问题，第一种情况会优先覆盖掉第二种情况所发生的问题

我们可以在成员调用时显式引用该成员所属具体类的作用域来解决上述描绘的二义性问题。我们可以将这种方式理解为基类指针（`this`）的引用实现（在 CPP 中并没有 `base` 指针）。参考下面代码所示：
...
- 基类的指针（引用）可用于接收一个子类实例的内存地址(***向上类型转换***)

- 当基类的指针（引用）所指向的是一个子类的实例在内存中的地址，那么这个基类指针可以强转为子类指针(***向下类型转换***)

为什么基类类型的指针可用于接收一个子类实例的地址？通过[继承的内存模型](#继承的内存模型)一节中我们能够知道，基类的成员数据会从其派生实例在内存中的首地址开始优先构建，在这之下才是各级派生所分布的内存成员数据。那么当我们用一个基类类型的指针去接收一个子类实例在内存中的首地址的时候是没有任何问题的，因为我们只是通过基类类型的指针去改变了解引用的步长为基类的长度，基于此，我们就能够保证对于指针的一切操作都能够限定在派生实例所属的内存段中，具体来说是这段内存中的基类成员数据所属那段范围内。反之，一个合法的指针（基类的指针所指向的地址是一个子类的实例）执行向下转换也是没有问题的。同理，此操作也仅仅只是改变了指针的解引用的步长，使指针的操作能够覆盖（扩大）到整个派生类所属的内存段内。这是一种***安全的转换关系***。

需要明白的是，指针的类型永远只是用来衡量解引用操作在内存段中所需的偏移量（步长），并不意味着指针的类型就代表了整个实例的本身。现在让我们思考一个问题：一个派生类类型的指针是否可以接收其基类实例在内存中的地址呢？答案是不可以的。由派生类所继承下来的来自基类中的成员是属于父与子二者之间的***共性***，基类的成员的数据往往能够、且足够被包含在派生类实例所属的那段内存空间内。然而作为派生，它往往需要与基类之间存在一些独属于它的***特性***，简单地说，一个派生类实例在构造后所需要的内存空间的长度往往要比基类要大。基于此，我们能够分析出当我们使用一个步长为派生类的指针去对一个内存长度小于或者等于派生类步长的基类实例做解引用操作的时候，就会发生内存越界的情况，是一种***不安全的转换关系***
...
<span id = "多继承与菱形继承的问题"></span>

#### 多继承与菱形继承的问题

通过在继承序列中同时指定多个基类以完成多继承的语义。在这种情况之下，派生类包含的每个基类都会被作为独立的实体，并按照继承顺序依次在派生类的内存布局中排列。一份参考代码与内存模型示意图如下所示：
...
class Object {
  std::int32_t m_postion;
};

class Player : public Unit, public Object {
  std::int32_t m_age;
};
```
<center>
<div style="color:orange; border-bottom: 1px solid #d9d9d9;
    ...
    color: #999;
    padding: 2px;">多继承代码参考</div>
</center>

```text
class Player    size(12):
        +---
 0      | +--- (base class Unit)
 0      | | m_state
        | +---
...
<div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">多继承内存模型示意图</div>
</center>


事实上，在多继承中基类这样的内存排列结构会带来更多的问题：当末端派生类直接继承了多个基类，而这些基类又同时派生自同一个根基类，由于这些基类会同时继承自根基类中的相同成员，所以当使用末端派生类的视角之下则会产生一个成员重复的问题。

成员重复并不是简单的，而是会衍生出其它诸多的问题。当仅只是间接引用基类中的重复成员，此时由于调用二义性问题的存在这将导致产生编译错误的问题。另外一点，也更是关键的一点，成员重复的问题会破坏面向对象的一些特性，正如下面的代码所示：末端派生不能同时拥有两个性别 `gender` 的抽象。虽然可以在语法层面去改变间接引用来解决前述问题，然而面向对象特性的根本性破坏是完全无法避免的，至少在不做任何多余工作的情况下。
...
虚继承是一种解决菱形继承问题的机制。当一个类以虚继承的方式继承另一个类时，我们称原始的类为虚基类，而继承它的类为虚派生类。区别于多继承，编译器能够确保继承自同源的一个或多个虚派生类的最终派生其实例内存中仅包含唯一一份虚根基类的实例（也就是虚基类中的成员数据只被实例化一次）以避免重复的成员状态。

<span id = "如何编写虚继承"></span>

**_如何编写虚继承_**

实现虚继承相对简单，只需在继承语句前添加 **_virtual_** 关键字即可维护一段虚基类与虚派生类的关系。需要特别注意的是，在使用虚继承时构造函数的调用方式和先前会存在一些差异。为了确保在继承链中即便虚基类被多次继承，其实例仍然是唯一且共享的，虚基类的构造函数仅能由最底层的派生类调用。这样的设计是必要的，如果允许多个派生类来完成同一个虚基类的构造将无法保证其实例的唯一性。当最底层派生类的实例被构造时，编译器会忽略掉所有虚派生类（即最底层派生类的直接基类）对虚基类构造函数的调用。然而，这样的实现机制并不意味着虚派生类中对于虚基类的构造函数调用是无效的，相反，为了确保虚基类中的成员数据在任何情况下都能够正确初始化，仅当创建虚派生类新实例时，指定对虚基类构造函数的调用将能够正常执行。

```cpp
class Animal {
public:
  // 如果没有手动创建默认的无参构造函数，那么
  // 往下的派生 Dog 和 Cat 则需要显式添加一
  // 个 "可能无用" 的基类构造函数的调用
  Animal() { } 
  Animal(const int32_t age) { m_age = age; }
...
public:
  Dog(const int32_t val) : m_val_dog(val) { }
...
public:
  Cat(const int32_t val) : m_val_cat(val) { }
...
class Hybird : public Dog, public Cat {
public:
  Hybird() : Dog(0x111), Cat(0x222), Animal(0x333) { }
  
private:
  int32_t m_val_hybird;
};
```
<center>
<div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">虚继承代码参考</div>
</center>

<span id = "虚继承内部实现"></span>

**_虚继承内部实现_**

我们能够在[标准](https://en.cppreference.com/w/cpp/language/derived_class)中找到关于虚继承的描述。需要注意的是，标准仅规范的是行为却不包含实现，故，如果要上升到如何实现这一特性的任何细节，这往往需要视编译器而定。然而在这个过程中有两个大家普遍所认可的概念：**vbptr** ($\text{Virtual Base Pointer}$) 和 **vbtable** ($\text{Virtual Base Table}$)。

虚派生类被构造的时候，编译器会把从虚派生类首地址开始到虚基类首地址所需偏移量都整合到一个名为 vbtable 的表格中。这个特殊的表格作为绑定到虚派生类自身的一个属性，它允许相同类类型的所有实例间共享。请注意，这并不是一种抽象的概念，**vbtable** 会实际存在于进程运行时的内存中，其大小需要视虚基类中所声明的成员数量而定。为了保证安全性，没有任何理由允许这块表格所在的内存段拥有可写属性，因此或许放在一个静态常量区中或许是一个合理的实现（在 GCC 的实现中会将 **vbtable** 放置在 **data.rel.ro** 段当中）。

当一个类类型拥有了 vbtable 的绑定后，对象的内存模型则发生了改变，参考[继承的内存模型](#继承的内存模型)中的表述，（虚）基类的数据在派生类实例的内存布局中不再初始化在起始部分，相反，它会被放置在实例内存的结尾部分。另外一点，对于虚基类中的成员引用方式相较于原始的固定静态偏移退化到两个步骤：首先，依据目标成员符号从 vbtable 中找到重定位所需的偏移；其次，取得这个偏移加上实例自身在内存首地址的结果，将其作为成员地址的最终索引。

到这里，我们还剩下一份工作没做，即虚派生类要如何在已知运行时信息欠缺的情况下知道所绑定的 vbtable 在哪个位置呢？特别是，指针类型的引入又为这个问题增加了更多的复杂性。为解决这一问题，现代编译器引入了一个指向 vbtable 的指针，即 vbptr。前面描述到，作为虚基类会被反向初始化到实例内存的尾部，而那一部分空余出来的内存部分则使用 vbptr 替代。**值得注意的是，即使是相同类型的多个实例，其 vbptr 都是恒等且指向一个唯一的 vbtable。** 这样的设计不仅确保每个实例都能访问到正确的虚函数实现，而且为实现 RTTI（$\text{Run-Time Type Information}$）提供了坚实的基础，使得我们可以在运行时安全地识别和处理对象的实际类型。虽然在某种程度上存在一部分冗余，但是产生的好处促使编译器更愿意接受这样的成本。

```text
class Hybird    size(48):
        +---
 0      | +--- (base class Dog)
 0      | | {vbptr}
 8      | | m_val_dog
...
Hybird::$vbtable@Dog@:
 0      | 0
 1      | 40 (Hybirdd(Dog+0)Animal)

Hybird::$vbtable@Cat@:
 0      | 0
 1      | 24 (Hybirdd(Cat+0)Animal)
...
<div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">虚继承内存模型示意图</div>
</center>


一般来说，vbtable 其内部的所有条目都要求拥有 **X**d**Y**vbptr**Z** 的命名规则，这样，我们就能将这个条目表示为：在 X 类中，从 Y 的 vbptr（起始地址）开始到 Z 类的入口处所需的偏移量。以下图为例，其中：

- DdBvbptrA = 16：在 D 类中，D 的 vbptr 到 A 类入口的偏移量为 40

- DdCvbptrA = 24：在 D 类中，C 的 vbptr 到 A 类入口的偏移量为 24

<center>
    <img style="border-radius: 0.3125em;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src="https://raw.githubusercontent.com/NGPONG/Blog/master/img/2024-05-10-14-04-41.png">
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;"> vbtable条目示例</div>
</center>

<br/>

要知道，仅维护虚基类与虚派生类二者之间的关系除开不能完全发挥虚继承的实际效用外还会增加额外的调用开销。引入一个派生自（并不是通过虚继承的方式）多个虚派生类的类则会显得不同，在构造该类时编译器会为我们保证继承链中出现的多个重复虚基类在实例内存的末端唯有且仅有一份实例。这份唯一的内存实例是解决菱形继承中二义性问题的关键因素。依赖于 vbptr 和 vbtable 的协作机制，即使虚基类的内存布局并不遵循普通继承模型的规则，我们也能够访问到在虚基类中的正确目标实现。

不可否认的是，vbptr/vbtable 的引入确实增加了一些额外的存储与调用性能的开销，那么为可能出现的二义性问题，似乎大可不必在一个完整类的继承层次结构在编译期间已经确定的情况下又退化到运行时绑定的工作。和 vbptr 类似的是，同样是因为指针的引入又让这些问题变得更加复杂。

考虑一个场景：我们有一个复杂的继承层次结构，类 B 和 C 都虚拟继承自类 A，类 D 则继承自 B 和 C。假设我们有一个函数，它需要 B* 类型的参数，并且返回值类型为 A* 。函数的输入参数必须能够接受任何能够指向 B 类型的指针，这包括了 D 对象，抑或是其他派生自 B 的对象；当我们传入一个 D 类型的指针时，索引到 A 子对象需要经过三条路径；如果我们传入的指针由 B 所构造呢？差异性在这里就体现出来了。所以必须提供一种机制，使得含有 B* 的代码能找出 A 子对象的位置。这完全依赖于最终派生类型的继承层级，因此 vptr/vtable 确实是一种合理的机制。
...
虚函数是一种实现[多态](#里式转换)的方式；我们可以在基类中声明一个函数为虚，派生类在保留相同签名的前提下对其进行重写，此时不管以何种方式（采用[里氏转换原则](#里式转换)是一种常用的方式），只要保证函数调用者为重写函数的派生类实例，那么最终都会跳转到重写后的实现。
<span id = "如何编写虚函数"></span>

**_如何编写虚函数_**

为一个成员函数的声明前添加 **_virtual_** 关键字来完成能够完成虚函数的定义。当一个类中声明了虚函数，那么其往下延续的派生都可以 <font color = "red">**_有选择性地_**</font> 对它重写。

对于虚函数的重写其实很简单，**只需要在派生类中保留一个签名一致**（甚至可以保留 ***virtual*** 关键字）（与访问修饰符无关，是的这确实会破坏抽象封装）**的成员函数即可完成对上级基类中的虚函数重写**。然而标准并不推荐这种方案，更正确的方法应当是在保留签名一致的基础上同时使用 ***override*** 关键字声明。这样，在保留重写特性的同时我们还能过获得额外的一些好处。除了能够显著提升代码可读性与继承结构顺序外，特别是，如果派生类的函数签名与其重写的基类虚函数不匹配时会引发编译错误。这种机制增强了编译期的错误检测能力，有助于减少程序运行时的错误，从而提高了软件的整体质量和稳定性。
...
在[标准](https://zh.cppreference.com/w/cpp/language/virtual)中所规定的 dynamic dispath 仅仅只是行为但不包含实现，因此如何实现这一特性的具体细节往往取决于编译器的设计。然而在这个过程中有两个大家普遍认可的概念：***vfptr***（$Virtual\ Function\ Pointer$）和 ***vftable***（$Virtual\ Functio\ Table$）

编译器会将一段继承链关系中声明的所有虚函数的实际地址组织起来并放入到一个名为 vftable 的表格中，我们可以将这个特殊的表格视为类本身的一个属性，也就是说，它会绑定到一个具体的类型，且允许相同类型的所有实例间共享。vftable 会实际存在于进程运行时的内存中，其大小需要视继承链中所声明的虚函数数量而定；然而具体到 vftable 所在的内存区域的问题，由于标准中并不包含这项实现因此我们也无法完全确定，但为了保证安全性，没有任何理由允许这块表格所在的内存段拥有可写属性，因此或许放在一个静态常量区中是一个合理的实现（在 GCC 的实现中会将 vftable 放置在 `data.rel.ro` 段当中）。

一份出自基类中初始状态下（承载着基类中所有虚函数实际地址）的 vftable 会沿着继承链往下传递，派生类会将传递下来的 vftable 拷贝下来并将这份新的 vftable 绑定到自身的 “属性”，随后对其进行修改：如果派生类重写（override）了任何虚函数，相应的函数地址会在 vftable 的某个表项中的反应到这种修改。此过程会在每一级派生类中重复进行，直至继承链的末端。每一级的派生类都会根据自身重写的需要更新和调整继承来自直接基类的 vftable 的拷贝，确保在对象的方法调用时，能够正确地引用到最新的实现。

当我们完成了这段 vftable 与类型之间的绑定工作后，实现多态就变得非常直观。继承链中的每个类类型都拥有一份唯一且从根基类继承下来并交由当前节点（类）所修改后的 vftable。。这样，在运行时当我们调用一个虚函数，程序就可以通过当前实例的类型对应的 vftable，结合条目所指示的偏移量来动态确定调用函数的最终实现。这个动态决定的过程就是多态的实现核心。

但请不要过于乐观，CPP 是一种较低级别且原生的语言，在运行时它仅包含有限的类型信息。那么如何查找一个实例所对应的类型则成为了一个很大的问题，特别是指针的引入而又增加了此问题更多的复杂性。为解决这一问题，现代编译器引入了一个指向 vftable 的指针，即 vfptr。当一个类被实例化的时候，如果编译器检测到当前类亦或是继承链中从根基类到当前类中含有虚成员，那么就会在其构造后的内存结构的首部中插入一个指向与当前类型绑定的 vftable 的指针 vfptr。在完成这样的准备工作后，每当实例（某个类型）需要调用一个虚函数，编译器都会在执行前查询当前类型所绑定的 vftable 对应调用函数的条目，这个条目所指向的内容则为调用虚函数的最终重现实现。值得注意的是，即使是相同类型的多个实例，其 vfptr 都是恒等且指向一个唯一的 vftable。这样的设计不仅确保每个实例都能访问到正确的虚函数实现，而且为实现 RTTI（$Run-Time\ Type\ Information$）提供了坚实的基础，使得我们可以在运行时安全地识别和处理对象的实际类型。虽然在某种程度上存在一部分冗余，但是产生的好处促使编译器更愿意接受这样的成本。
...
int main (int argc, char *argv[]) {
  Object1 *obj1 = new Object2();
  obj1->bar(); // call Object2::bar
  obj1->zed(); // call Object2::zed

  Object1 *obj1 = new Object1();
  obj1->bar(); // call Object1::bar
  obj1->zed(); // call Object1::zed

  Object0 *obj2 = new Object2();
  obj2->foo(); // call Object0::foo
...
<div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">虚函数代码参考</div>
</center>

```
 class Object0   size(16):
        +---
 0      | {vfptr}
 8      | m_val0
12      | m_val1
...
============================================================================
 ...
 class Object1   size(32):
        +---
 0      | +--- (base class Object0)
 0      | | {vfptr}
 8      | | m_val0
12      | | m_val1
        | +---
...
Object1::$vftable@:
 ...
 class Object2   size(40):
        +---
 0      | +--- (base class Object1)
 0      | | +--- (base class Object0)
 0      | | | {vfptr}
 8      | | | m_val0
12      | | | m_val1
...
public:
  virtual void foo() {
    foo();          // 由于调用当前函数的实例的类型为 Object1 且该类型完成了对
                    // 当前函数的重写，因此，在不使用命名空间引用作用域的情况下
                    // 默认都会调用到重写后的函数 Object1::foo()

    Object0::foo(); // 除非刻意指定命名空间
  }
...
public:
  virtual void foo() {
    Object0::foo(); // 根据命名空间显示指定调用基类中的虚函数
  };
  
  int m_val2 = 3;
  int m_val3 = 4;
...
<div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
    padding: 2px;">一段会产生无限递归死循环的使用案例</div>
</center>
...
[前述](#继承链中类的构造和析构顺序)中我们描绘道，编译器通过在类的析构函数中插入一段转发自基类析构的调用以保证一整条继承链的析构顺序。因为某些原因导致一个非 POW 类型的对象需要被结束生命时，编译器则会拿到声明该对象时所使用的类型并调用其析构函数以完成内存数据的释放工作；其中，指针/引用类型的对象是特殊的，在执行这种操作时编译器会省略掉指针/引用的声明而直接调用原始类型下的析构函数。

正是因为指针/引用类型的这种特殊性，使运行时所构造的多态在释放时则会遇到一些问题。考考虑一个情景，我们有一个基类 A 和其派生类 B。为实现多态，我们必将需要使用类似于 `A *a = new B();` 的语义。那么在释放该对象时，编译器在省略掉指针/引用语义后所拿到的原始类型结果为 A，也就是说在这种条件之下编译器仅能完成类型 A 的虚构调用而忽略掉其派生 B 的。这样的结果，不仅使原始逐层级析构的特性失效了，更危险的是，如果类型 B 中拥有堆存储期的成员数据，那么无法析构也就意味着内存泄漏的发生。

我们无法变更编译器对于指针/引用类型对象的析构函数的调用惯例。一个想法就是我们可以重写类型 A 中的析构函数，使其变更为我们所需要的派生实现。依赖于派生类中析构函数本身保有的能够沿继承链逐层向上调用的能力就能够修复原始继承链失效的问题，进而解决可能出现的内存泄漏的问题。下面是一个代码示例：
...
纯虚函数是指将声明的虚函数实现为 0 的函数。它要求作为下级派生必须要对纯虚函数进行重写，否则会导致编译错误。另外一点，当一个类拥有了一个纯虚函数后，编译器会将其认作为抽象类。抽象类无法创建自身且不能拥有任何声明，这包括了：普通的声明、函数的形参、返回值，然而抽象类的指针是能够接受的，即，这是强制开发人员使用多态构造的一种语法上的约束。
...
需要注意的是，虚析构也能够作为纯虚函数，然而为了保证继承体系中虚构函数逐层顺序的调用要求，所以定义的纯虚析构需要拥有实现（可以在全局作用域下），否则同样会出现编译失败的问题。这种语义对于纯虚函数本身的确看起来很别扭，似乎纯虚析构其实现赋值为 0 并无太大意义。的确如此，纯虚析构所保证的仅仅只是能够让当前所在类声明为一个抽象类，并且保证其下级派生必须对析构进行重写仅此而已。
